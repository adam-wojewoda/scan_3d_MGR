...
def read_single_register(address):
        # make sure we have the write command on address
    if start_address>0b11111111:
        raise ValueError("Address too big")
    if start_address<0b10000000: # Write read_command_bit
...
def reg_series_to_val(in_list):
    ...
    out_fin = [None]*6
    for i in range(3):
        out_fin[i]=out_raw[i]*gyro_multiplier
        out_fin[i+3]=out_raw[i+3]*accel_multiplier#*multiplier
...
def live_plotter(x_vec,y1_data,y2_data,y3_data,y4_data,y5_data,y6_data,
                 line1,line2,line3,line4,line5,line6,identifier='',pause_time=0.1):
    if line1==[]:
        ...
        line2, = ax.plot(x_vec,y2_data,'-o',alpha=0.5)
        line3, = ax.plot(x_vec,y3_data,'-o',alpha=0.5)
        line4, = ax.plot(x_vec,y4_data,'-o',alpha=0.5)
        line5, = ax.plot(x_vec,y5_data,'-o',alpha=0.5)
        line6, = ax.plot(x_vec,y6_data,'-o',alpha=0.5) 
...
y5_vec = np.random.randn(len(x_vec))
y6_vec = np.random.randn(len(x_vec))
line1 = []
line2 = []
line3 = []
line4 = []
...
while True:
    ...
    line_temp = live_plotter(x_vec,y1_vec,y2_vec,y3_vec,y4_vec,y5_vec,y6_vec,
